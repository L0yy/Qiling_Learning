...
class Qiling(QlCoreHooks, QlCoreStructs):    
    def __init__(
        ...
        self._platform = ostype_convert(platform.system().lower())#利用map和枚举量确定架构
        self._internal_exception = None
        self._uc = None
        self._stop_options = QlStopOptions(stackpointer=stop_on_stackpointer, exit_trap=stop_on_exit_trap)#指定停止方式
        ...
        self._targetname = ntpath.basename(self._argv[0])#获取文件名
